...
const VisitadorToolbar = ({
    ...
    // Selección
    selectedIds = [], // Cambiado para manejar el array de IDs directamente
    onSelectAll,
    currentItems = [],
    onDeleteSelected,
}) => {
    return (
        <div className="fixed top-20 left-0 right-0 z-50 bg-white border-b border-slate-200 w-full shadow-sm px-4 py-2">
            <div className="flex items-center justify-between gap-2 overflow-x-auto lg:overflow-visible no-scrollbar">
                ...
                {/* 1. SECCIÓN IZQUIERDA: SELECCIÓN Y BUSCADOR */}
                <div className="flex items-center gap-3 min-w-fit">
                    <div className="flex items-center gap-3 border-r border-slate-200 pr-3">
                        <label className="flex items-center gap-2 cursor-pointer group">
                            <input
                                type="checkbox"
                                checked={currentItems.length > 0 && currentItems.every(v => selectedIds.includes(v.id))}
                                onChange={e => onSelectAll(e, currentItems)}
                                className="w-5 h-5 rounded border-slate-300 text-[#3D3FD8] focus:ring-[#3D3FD8] cursor-pointer transition-transform group-hover:scale-105"
                            />
                ...
                <div className="flex items-center gap-2 min-w-fit">
                    {/* Botón Borrar (Habilitado/Deshabilitado según selección) */}
                    <button
                        ...
                        className={`${selectedIds.length > 0
                            ...
                            } px-3 py-2 rounded-lg font-black text-[10px] uppercase border transition-all flex items-center gap-1.5`}
                    >
