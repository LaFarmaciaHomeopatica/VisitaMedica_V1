...
const Gmedicos = ({ auth, medicos = [], visitadores = [], tiposDocumento = [], categorias = [] }) => {
    ...
    const [isExporting, setIsExporting] = useState(false); // ← Estado para el modal de carga de exportación
    ...
    // --- Handlers ---
    const executeExport = () => {
    // 1. Cerramos el modal de confirmación y abrimos el modal de carga
    setIsExportModalOpen(false);
    setIsExporting(true);

    // 2. Generamos un token único de identificación
    const downloadToken = new Date().getTime().toString();

    const form = document.createElement('form');
    form.method = 'POST';
    form.action = route('Gmedicos.exportar');
    ...
    // 3. Adjuntamos el token al formulario HTML
    ...
    if (selection.selectedIds.length > 0) {
        ...
        idsInput.name = 'ids';
        idsInput.value = selection.selectedIds.join(',');
    ...
    // 4. Verificación continua (Polling):
    // Se ejecuta cada 300ms revisando si la cookie 'download_token' ya fue escrita por Laravel
    const checkCookieInterval = setInterval(() => {
        if (document.cookie.indexOf(`download_token=${downloadToken}`) !== -1) {
            // ¡La descarga inició en el navegador! Cerramos el modal
            setIsExporting(false);
            clearInterval(checkCookieInterval);

            // Limpiamos la cookie borrándola del navegador
            document.cookie = `download_token=${downloadToken}; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;`;
        }
    }, 300);
};
...
// Componente modal de carga para la exportación
const ExportLoadingModal = () => (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in">
        <div className="bg-white p-8 rounded-xl shadow-2xl flex flex-col items-center max-w-sm w-full mx-4 border border-gray-100">
            ...
            <h3 className="text-lg font-semibold text-gray-800">Generando archivo Excel...</h3>
            <p className="text-sm text-gray-500 text-center mt-2">
                Estamos preparando tus datos. La descarga comenzará automáticamente en unos instantes.
            </p>
        </div>
    </div>
);
