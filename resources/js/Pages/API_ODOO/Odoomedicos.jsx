...
import {
    ...
    FaCircleNotch, FaSpinner, FaFileInvoiceDollar, FaReceipt
...
export default function OdooMedicos({ auth, conexionEstado = 'sin_probar', flash, errors }) {
    ...
    const totalVentas   = transacciones.filter(tx => tx.origen.includes('Venta'))  .reduce((acc, tx) => acc + (Number(tx.total) || 0), 0);
    const totalFacturas = transacciones.filter(tx => tx.origen.includes('Factura')).reduce((acc, tx) => acc + (Number(tx.total) || 0), 0);
                            ...
                            {resultados.length > 0 && (
                                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                                    ...
                                    <div className="px-6 py-3 bg-slate-800 flex items-center justify-between flex-wrap gap-3">
                                        {/* Total del filtro activo — prominente */}
                                        <div>
